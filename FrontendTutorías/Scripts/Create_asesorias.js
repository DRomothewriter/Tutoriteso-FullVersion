...
document.getElementById('formAsesoria').addEventListener('submit', async function (e) {
  ...
  const API_URL = 'http://localhost:5500/api/asesorias';

  // Captura los valores del formulario
  const materia = document.getElementById('materia').value;
  const plataforma = document.getElementById('plataforma').value;
  const plataformaPersonalizada = document.getElementById('plataformaPersonalizada').value;
  ...
  if (plataforma === 'Otro' && plataformaPersonalizada.trim() === '') {
    return alert('Debe especificar una plataforma personalizada si selecciona "Otro".');
  }
  ...
  if (plataforma === 'Otro') {
    body.plataformaPersonalizada = plataformaPersonalizada;
  }

  console.log("Cuerpo enviado al backend:", body); // Útil para debug
